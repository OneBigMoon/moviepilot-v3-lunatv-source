...
- 搜索结果会优先调用 MoviePilot 原生识别链关联 TMDB；TMDB 能提供完整季集数时，会帮助拆分平铺的多季合集。
- 下载目录留空时按电影/电视剧自动读取 MoviePilot 的本地目录设置；填写插件目录则优先使用插件目录。
- 任务通知通过 MoviePilot 插件消息能力发送；MoviePilot 的媒体库扫描可以继续接管已完成文件。

## 目录与命名

下载目录可以由插件设置指定，也可以留空自动复用 MoviePilot 的本地下载目录。插件不会修改系统目录配置；可开启“下载后调用 MoviePilot 整理链”将文件转入系统媒体库规则，也可以直接写入指定目录。
