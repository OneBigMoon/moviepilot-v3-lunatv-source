...
- 新增或端点变化的来源会记录搜索协议健康状态；API 1002 等“源站在线但限制关键词搜索”的响应会明确标记为“搜索受限”，网络失败只累计诊断信息并保留调用机会，直到用户配置禁用。合法的空搜索列表只表示本次关键词未命中，不会被误判为故障。
...
- 选择单个资源时会保留同集的其他来源作为备用；当前地址下载失败后自动按清晰度/来源顺序逐个换源，候选耗尽才标记失败。“所有匹配源都排队”仍按原行为分别创建任务。
...
- MoviePilot 活跃电视剧订阅默认每 30 分钟检查一次（可配置 5–1440 分钟）；首次补齐所有缺集，后续只加入新增集；选择来源时先保证最新季集完整，再比较清晰度。队列间隔支持 1–1440 分钟，请求超时支持 1–60 秒。
...
同一集只会进入一次任务；失败后重新提交会复用原任务，MoviePilot 重启时会清理旧版本遗留的重复记录，并把真正中断的任务恢复为待处理。升级前已经创建、但没有备用来源记录的旧失败任务不会凭空获得候选源，需要重新搜索并提交一次资源。详情接口缺少播放地址时，插件会自动补查 Apple CMS `ac=detail`，并识别 `S01E01`、`第 8 集`、`第 1 季` 等标记。只有线路或 TMDB 季集数能确定边界时才会按季聚合 `1-8 季`，无法确认时才退回单集，避免静默错放。
