# LunaTV 资源订阅（MoviePilot V3）

独立的 MoviePilot V3 插件，读取 MoonTV/LunaTV 的 `api_site` 配置，接入标准苹果 CMS V10 资源站。

## 当前范围

- 读取可配置的 LunaTV JSON 配置地址；默认使用个人可维护的配置仓库。
- 资源站按 allowlist 启用，单个源失败不阻塞其它源。
- 使用 MoviePilot 活跃订阅名称搜索资源。
- 注册为 MoviePilot V3 原生“探索”数据源，搜索结果带有稳定的 `lunatv` 媒体身份，可继续创建订阅。
- 任务严格串行执行，不启动并行下载。
- 支持 m3u8/HTTP 资源下载到指定目录，或生成 `.strm`。
- 默认调用 MoviePilot 原生整理链；插件只在宿主链不可用时保留直写结果，并保留插件内任务历史。
- 下载完成后沿用 MoviePilot 已启用媒体服务器设置触发媒体库刷新；播放不放在插件页，仍由既有媒体服务器页面负责。
- 自动复用 MoviePilot“智能助手配置”（DeepSeek 等 OpenAI 兼容模型）清理片名后再搜索；未配置或调用失败自动回退原名称。
- 搜索结果自动调用 MoviePilot 原生识别链关联 TMDB；TMDB 能提供完整季集数时，会帮助拆分平铺的多季合集。
- 搜索结果会显示默认 TMDB 关联；同名或多季作品可重新搜索候选并手动切换，所选作品的季集数会参与安全拆分。
- 下载目录留空时按电影/电视剧自动读取 MoviePilot 的本地目录设置；填写插件目录则优先使用插件目录。
- 任务通知通过 MoviePilot 插件消息能力发送；MoviePilot 的媒体库扫描可以继续接管已完成文件。

## 目录与命名

下载目录默认自动复用 MoviePilot 的本地目录设置；只有需要覆盖宿主目录时才填写插件里的可选目录。插件不会复制或修改系统目录配置，下载后直接交给 MoviePilot 原生整理链。

```text
电影名 (年份)/电影名 (年份).mp4
剧名 (年份)/Season 01/剧名 (年份) - S01E01.mp4
```

下载先写入 `.part`，成功后再改为正式文件名，避免媒体库扫描到半成品。目录内没有正在下载的缓存文件时，媒体库才会显示完整文件夹；MoviePilot/Emby 的自动监控建议在确认前暂时关闭。

同一任务只会进入一次队列；队列每次只执行一个任务，不会并行下载；MoviePilot 重启后会把中断中的任务恢复为待处理。详情接口缺少播放地址时，插件会自动补查 Apple CMS `ac=detail`，并识别 `S01E01`、`第 8 集`、`第 1 季` 等标记。只有线路或 TMDB 季集数能确定边界时才会自动拆分 `1-8 季`，无法确认时会提示人工处理，避免静默错放。

## 配置

默认配置地址：

```text
https://raw.githubusercontent.com/hafrey1/LunaTV-config/main/LunaTV-config.json
```

建议先只启用自己确认可用、且有权访问的资源源。插件不会绕过 DRM、登录保护或付费限制。

使用顺序：在插件卡片里保存源地址并启用原生桥接 → 在 MoviePilot 原生“探索”中选择 `LunaTV / 苹果 CMS` 搜索并创建订阅 → 由 MoviePilot 原生订阅和整理链管后续流程。插件不增加独立订阅导航。目录、智能助手、TMDB、媒体服务器无需在插件中重复配置。可选目录覆盖必须是容器内路径，不能填宿主机未映射的路径。

插件工作台只负责诊断、排队、整理状态和历史，不内置 m3u8 播放器。你已有 Emby 时，只需按 MoviePilot 原生媒体服务器设置完成映射，插件会沿用已启用的媒体服务器并自动请求同步。

## 开发检查

```bash
python3 -m pytest tests
python3 -m compileall plugins.v3/lunatvsource
git diff --check
```
