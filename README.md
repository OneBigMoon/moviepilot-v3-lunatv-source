...
使用顺序：在插件卡片里保存订阅地址并启用原生桥接 → 使用 MoviePilot 顶部全局搜索查找媒体并进入原生详情页 → 通过“搜索资源”或原生订阅触发 LunaTV 资源匹配 → 由 MoviePilot 原生订阅和整理链管后续流程。插件不增加独立探索或订阅导航。目录、智能助手、TMDB、媒体服务器和链接权限无需在插件中重复配置。
