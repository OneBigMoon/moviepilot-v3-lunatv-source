...
同剧各季的目录年统一取 TMDB 作品首播年：CMS 每季行常写该季播出年，直接沿用会把各季拆进 `标题 (2017)`、`标题 (2018)` 这类不同目录，绿联便显示成同剧多条目。订阅刷新、手动下载和识别链都已统一到作品年；TMDB 关联不上时只能沿用 CMS 年份，无法保证合并。

电影整理成功后，插件会在后台把 TMDB 的 `belongs_to_collection` 补写进目标 `movie.nfo` 的 `<set>`（含 `<collectionnumber>`），绿联/Emby 的合集页据此分组同系列电影。宿主刮削由整理事件异步落盘，补写会等待 NFO 出现并有界重试；已有 `<set>` 的 NFO 不会被改动。刮削关闭、整理失败回退直写或 STRM 模式都不补写。

已有错误条目不会由插件自动迁移。应先让 MoviePilot 覆盖旧 NFO，必要时备份并删除旧的 `tvshow.nfo`、`season.nfo` 和单集 NFO，再在绿联中执行完整覆盖或重新识别。升级前按旧年份分季落盘的目录同样需要删除后重新下载，才会合并进作品年目录。
